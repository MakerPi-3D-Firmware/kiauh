...
function download_fluidd_macros() {
  local fluidd_cfg="https://raw.githubusercontent.com/MakerPi-3D-Firmware/FluiddPI/master-sg/src/modules/fluidd/filesystem/home/pi/klipper_config/fluidd.cfg"
  local configs path
...
function get_fluidd_download_url() {
  ...
  latest_url="https://github.com/MakerPi-3D-Firmware/fluidd/releases/download/${latest_tag}/fluidd.zip"

  ### get stable fluidd download url
  stable_tag=$(echo "${tags}" | grep -E "^v([0-9]+\.?){3}$" | head -1)
  stable_url="https://github.com/MakerPi-3D-Firmware/fluidd/releases/download/${stable_tag}/fluidd.zip"
...
type: web
channel: stable
repo: MakerPi-3D-Firmware/fluidd
path: ~/fluidd
MOONRAKER_CONF
